    ...
    def update(self, delta_frames=1):
        ...
        if self.msec_to_climb > 0:
            ...
            self.y -= (Bird.CLIMB_SPEED * frames_to_msec(delta_frames) *
                       (1 - 0.9*np.cos(frac_climb_done * np.pi)))
...
class PipePair(pygame.sprite.Sprite):
    ...
    def __init__(self, pipe_end_img, pipe_body_img):
        ...
        # Pick a random height for the bottom piece
...
def main_loop(flappy_bird_AI,silent=False,gamble_chance=0,display=True,display_surface=None,images=None):
    ...
    if type(display_surface) == type(None):
    ...
    # Load the Q table and initialize the state vector
    old_state = [0,0,0,1]
    n_penalised = 12
    old_states = n_penalised*[old_state]
    last_actions = n_penalised*[False]
    ...
    while not done:
        ...
        if (frame_clock % UPDATE_EVERY) == 0:
            # First see which pipe we need
            if len(pipes) == 1:
                last_pipe = pipes[-1]
            elif pipes[-2].visible:
                last_pipe = pipes[-2]
            else:
                last_pipe = pipes[-1]

            # Get the current state
            new_state = flappy_bird_AI.get_state(bird,last_pipe)

            # Don't update on the first frame...
            if frame_clock >0:
                # Was the last decision a good one?
                state_alive_1 = 1 # 0 if dead, 1 if alive, but if we got to here then we're alive
                last_action_ix = 1*last_actions[-1]
                reward = [-1000,1][state_alive_1]
                flappy_bird_AI.update_qtable(old_states[-1],new_state,last_action_ix,reward)


            # Choose the next action to maximizes the possible reward
            if frame_clock == 0: # Do nothing in the first tick
                action = False
            else:
                action = flappy_bird_AI.get_best_action(new_state)

            # And save the current state for next time
            old_states.pop(0)
        ...
        # Update the bird with the action
        if action:
            bird.msec_to_climb = Bird.CLIMB_DURATION

        # And leave a way for us to escape...
        ...
        # comment this out to not display it
        if display:
            pygame.display.flip()
        # print(action)
    ...
    # Update the q_table based on our failure
    reward = -1000
    new_state = flappy_bird_AI.get_state(bird,last_pipe)
    old_states.append(new_state)
    # Penalize the last n_penalised states
    for ix in np.arange(n_penalised,0,-1):
        last_action_ix = 1*last_actions[ix-1]
        flappy_bird_AI.update_qtable(old_states[ix-1],old_states[ix],last_action_ix,reward)
